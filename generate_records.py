...
marital = ['married'] * 55 + ['unmarried'] * 25 + ['widow'] * 7 + ['widow'] * 13
age = np.random.randint(1, 19, size=(1, 20)).tolist()[0]  + np.random.randint(18, 31, size=(1, 46)).tolist()[0] +  np.random.randint(30, 61, size=(1, 25)).tolist()[0] + np.random.randint(60, 100, size=(1, 10)).tolist()[0]  
...
energy = np.random.randint(0, 1000, size=(1, 30)).tolist()[0]  + np.random.randint(1000, 2000, size=(1, 25)).tolist()[0] +  np.random.randint(2000, 3000, size=(1, 20)).tolist()[0] + np.random.randint(3000,4000, size=(1, 10)).tolist()[0]  
...
cities = ["Hyderbad","Bangalore","Mumbai","Kolkata","Chennai","Jaipur","Delhi","Lucknow"]*4 + ["Chandigarh","Srinagar","Panaji","Jammu","Bhubaneswar","Chandigarh","Vijayawada"]*3 + ["Itanagar","Dispur","Patna","Raipur","Gandhinagar","Shimla","Ranchi"]*4 + ["Thiruvananthapuram","Bhopal", "Imphal","Shillong","Aizawl","Kohima","Gangtok","Agartala","Dehradun"] * 2+ ["Khammam"]
education = np.random.randint(0, 13, size=(1, 12)).tolist()[0]  + np.random.randint(12, 15, size=(1, 30)).tolist()[0] +  np.random.randint(14, 19, size=(1, 35)).tolist()[0] + np.random.randint(18, 25, size=(1, 23)).tolist()[0]
...
df.to_csv("/Users/nikki/Desktop/personality_survery.csv", encoding='utf-8', index=False)
